...
namespace
{
const auto lang = TextEditor::LanguageDefinition::Ruby();

bool LoadedWorkspaces = false;
uint32_t CurrentWorkspace = 0;
std::map<uint32_t, std::string> Workspaces;
uint64_t FlashStartTime = timer_start();

std::string current_workspace_name()
{
    return "workspace_" + string_number_name(CurrentWorkspace);
}
...
void TextEditor::HandleKeyboardInputs()
{
    ...
    if (ImGui::IsWindowFocused())
    {
        ...
        // Sonic pi specific
        if (alt && io.KeysDown[SDL_SCANCODE_R])
        {
            edit_run();
            EnsureCursorVisible();
            return;
...
static bool TokenizeRubyArg(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
    const char* p = in_begin;

    auto isChar = [](auto p) { return ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') || (*p >= '0' && *p <= '9') || *p == '_'); };
    if (*p == ':')
    {
        p++;
        while (p < in_end && isChar(p))
            p++;
        if (p < in_end)
        {
            out_begin = in_begin;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Ruby()
{
    static bool inited = false;
    static LanguageDefinition langDef;
    if (!inited)
    {
        static const char* const keywords[] = {
            "__FILE__", "and", "def", "end", "in", "or", "self", "unless", "__LINE__", "begin", "defined?", "ensure", "module", "redo", "super", "until", "BEGIN", "break", "do", "false",
            "next", "rescue", "then", "when", "END", "case", "else", "for", "nil", "require", "retry", "true", "while", "alias", "class", "elsif", "if", "not", "return", "undef", "yield", "choose"
        };
        for (auto& k : keywords)
            langDef.mKeywords.insert(k);
        ...
        langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool {
            ...
            if (in_begin == in_end)
            {
                ...
                paletteIndex = PaletteIndex::CharLiteral;
            else if (TokenizeRubyArg(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::CharLiteral;
            else if (TokenizeCStyleIdentifier(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Identifier;
            else if (TokenizeCStyleNumber(in_begin, in_end, out_begin, out_end))
...
void set_workspace_text(uint32_t index, const std::string& text)
{
    Workspaces[index] = text;

    if (index == CurrentWorkspace)
    {
        editor.SetText(Workspaces[CurrentWorkspace]);
    }
}

void switch_to_workspace(uint32_t index)
{
    if (CurrentWorkspace != index)
    {
        Workspaces[CurrentWorkspace] = editor.GetText();
...
void edit_window_update(const MessageInfo& info)
{
    if (info.type == MessageType::SyntaxError ||
        info.type == MessageType::RuntimeError)
    {
        TextEditor::ErrorMarkers markers;
        markers.insert(std::make_pair(info.line, info.text + " : " + info.errorLineString));
        editor.SetErrorMarkers(markers);
    }
}

void edit_window_update(const BufferInfo& buffer)
{
    if (buffer.type == BufferType::Replace)
    {
        set_workspace_text(buffer.bufferIndex, buffer.content);
 
        editor.SetCursorPosition(TextEditor::Coordinates(buffer.line, buffer.index));
        // TODO: setFirstVisibleLine(first_line);
        LoadedWorkspaces = true;
    }
}

void edit_window_close()
{
    if (LoadedWorkspaces)
    {
        Workspaces[CurrentWorkspace] = editor.GetText();

        // This should be async to avoid the following sleep
        sonic.spApi->SaveWorkspaces(Workspaces);

        std::this_thread::sleep_for(1s);
    }
}

void edit_window_show()
{
    edit_window_init();
    ...
    {
        for (uint32_t i = 0; i < sonic.spApi->MaxWorkspaces(); i++)
        {
            bool selected = (i == CurrentWorkspace);
            ImGuiTabBarFlags tab_bar_flags = ImGuiTabBarFlags_None;
            auto label = fmt::format(" {0} ", i);
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImColor(.3f, .1f, .8f, 1.0f).Value);
...
void edit_run()
{
    FlashStartTime = timer_start();

    editor.SetErrorMarkers(TextEditor::ErrorMarkers{});
    sonic.spApi->SaveAndRunBuffer(current_workspace_name(), editor.GetText());
}
